...
//
// Раньше подменялась «ошибка» — латинская «o» вместо кириллической. Метки
// переведены на английский 2026-09-09, и «ошибка» удалена как дубль `bug`,
// но подмена никуда не делась: она просто поменяла направление. Теперь
// кириллические «о» и «е» подставляются в английское имя, и снимок обязан
// отличить их так же побайтово.
test('кириллическая «о» в «good first issue» — это другая метка, и она находится', () => {
  const real = realLabels();
  assert.ok(real.has('good first issue'), 'метка good first issue отсутствует в снимке');
  assert.ok(!real.has('gооd first issue'), 'снимок принял метку с кириллическими o как настоящую');
  assert.ok(!real.has('good first issuе'), 'снимок принял метку с кириллической e как настоящую');
  // И объявление формы совпадает побайтово, а не «на вид».
  const oshibka = readFileSync(join(FORMS, 'oshibka.yml'), 'utf8');
  assert.deepEqual(declaredLabels(oshibka), ['bug']);
});
