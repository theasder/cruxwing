...
describe('open-source furniture', () => {
  ...
  describe('connector form', () => {
    ...
    test('does not promise a label the repository does not have', () => {
      // `oshibka.yml` объявляет labels: ["ошибка"], а такой метки у репозитория
      // нет — issue приезжает без метки, и никто этого не замечает. Здесь метка
      // не объявляется вовсе, пока она не заведена: молчаливое обещание хуже
      // отсутствующего.
      assert.doesNotMatch(form, /^labels:/m,
        'форма объявляет метку — заведите её в репозитории и снимите эту проверку');
    });
  });
});
