...
системных запросов, свой словарь. 3700 тестов проходят.
...
Тесты: `cd app && swift test` (2833 штуки), mvp — 597 и `npm test` в корне (270 — про
...
Разработчику привычнее одной командой:

```bash
brew install --cask theasder/orakul/orakul
```

Кран (`theasder/homebrew-orakul`) **ещё не создан** — это единственное, чего не
хватает: сам каст собирается из репозитория, `bash scripts/refresh-cask.sh`
считает суммы по собранным образам и печатает готовый файл. В основной
...
падали на первой же строке. Теперь ядро и командная строка собираются в
контейнере `swift:6.0` на каждый пулл-реквест — отдельным заданием CI. Набор
тестов там пока не гоняется: он запускается, но три класса проверок на Linux
ведут себя иначе (права под root, Windows-1251, разбор URL). Разбор — в
...
Если хочется начать с готовой задачи — метка
