...
it. Measured that day: of 439 string literals in `Views/` and `Onboarding/`, 167
...
| **Local notes: Obsidian and any `.md` directory** | **Connected 2026-08-18**, now in the app too: a folder is chosen in Settings and kept as a security-scoped bookmark, and the source joins the fan-out during a call. No API, no token, no host — files read from disk, and unplugging the network changes nothing | Nothing blocking. Large vaults answered by §7.2's bound: 2000 files per question, freshest first, and the coverage travels into the prompt so a partial read cannot be quoted as a whole one. That last clause was half true until 2026-08-20: the coverage travelled **with findings**, and an empty search returned nothing at all — the one case where a partial read is quoted as a whole one, because the model hears silence and answers «в заметках этого нет». The reasoning was written beside that very line and applied only to the non-empty branch |
...
| Be the source nobody swept | Google Docs, Sheets and Drive ask for a **page**, and the page is tiny: three documents per question, ceiling five, each document's text taken from the front. So an empty result there means «not in the three we read», not «not on the Drive» — and it said neither, in both directions: nothing when it found nothing, and no bound when it found something. Every other source began naming its bound the same day; this one lives in `AppState` rather than in the fan-out, so the sweep and the new structural check both went past it | The same sentence travels either way, and it names what was read, the ceiling, and the fact that each text is truncated. The number in that sentence is pinned to the number actually requested, in the other file — a promise about a figure kept somewhere else is exactly what drifts unnoticed |
...
| A connector built from docs, never against a live service | Counted from the manifests rather than remembered: **ten** carry `liveCheckedOn` (BookStack, Gitea / Forgejo, GitLab, Matrix / Element, Mattermost, Nextcloud, Plane, Redmine, Rocket.Chat, Wiki.js) and **eleven** do not, plus the two still written by hand — Яндекс Трекер and Битрикс24. The row used to name Битрикс24 alone, which read as if it were the exception when it is the majority | For a hosted service, a live check by other hands (issue #1). For a **self-hosted** one no account is needed, only a container: `scripts/zhivaya-proba.sh` stands the service up, seeds it, searches, and removes it. Five of them looked stand-up-able that way; **Mattermost, Rocket.Chat and Matrix / Element are done — 2026-08-20**. The other two are not the same kind of «not yet», and saying so is the point of counting: **Zulip** runs as five services (Postgres, Redis, memcached, RabbitMQ, the server itself) described by the vendor's own `compose.yaml`. Attempted 2026-08-20 and **not finished** — the branch was written, run, and reverted rather than left half-working, because a probe that fails at the fourth step is worse than none: the three-list check would pass while the service did not. Four obstacles were found in order, and each is written down so the next attempt starts past them. (1) `DISABLE_HTTPS` and `SSL_CERTIFICATE_GENERATION` are refused by the 12.x image by name — an empty `CERTIFICATES` already means http-only. (2) memcached needs its SASL database **written by hand**: the image does not create one, `memcached -S` answers «Auth failure», and the server then refuses to start at all. The user names come from the vendor's compose — `zulip@$HOSTNAME` and `zulip@localhost`. (3) Passwords travel as `SECRETS_*` environment variables; the vendor's compose uses files, and files would be a fourth kind of litter after containers, networks and volumes. (4) The one still open: `EXTERNAL_HOST` refuses `localhost:3989` — «does not contain a domain part», hint «add .local» — so the server wants a name the probe then has to reach it by, and Zulip checks the Host header. That is where the next attempt begins. The line here first said it publishes amd64 images only; that was wrong and is worth keeping rather than quietly fixing. I read the architectures off `zulip/docker-zulip` on Docker Hub — the repository the vendor no longer publishes the server to. The image the compose file actually names, `ghcr.io/zulip/zulip-server`, carries **arm64**, as do the Postgres and RabbitMQ images beside it. Checking the wrong registry looks exactly like checking the right one, and the answer it gives is a fact of the same shape — which is how a measurement that was never taken ends up written down as one. **Outline cannot be probed this way at all**: the vendor states plainly that it «does not support email + password authentication natively», so the first user can only arrive through an external identity provider, and an API token is issued from a browser session. Reopening it needs an identity provider standing beside it — Dex in a container — which is a different piece of work from «stand the service up». The earlier line here said five were waiting; four were, and one was blocked. The rest are hosted-only (Slack, Linear, Trello, Kaiten, WEEEK, YouGile, Пачка, GitFlic) or licence-gated (Jira Data Center). The row said «four» for two days after the number was six, which is why §2.2's bold marks are derived from the manifests rather than remembered — and why these numbers are checked too |
...
**Every source that reaches the prompt is classified — 2026-08-20.** The «empty result
...
The first version of that check read only the fan-out, and the very next source
found by hand — Google Docs, Sheets and Drive — lived outside it, in `AppState`.
So the rule is stated where it belongs: every `sourceID` that can reach the
prompt, anywhere in the app, is named in one of two lists — «states its bound» or
«searches by itself» — with a reason, and both lists are checked against the code
in both directions. The two lists are cross-checked as well: a source obliged to
state its bound must have its own line, or the prefix of its neighbours excuses
it and the reason beside it quietly becomes false.
