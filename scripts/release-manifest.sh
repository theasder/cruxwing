...
artifacts=(orakul-AppleSilicon.dmg orakul-Intel.dmg)
...
mkdir -p "$parent"
stage="$(mktemp -d "$parent/.orakul-release-${tag}.XXXXXX")"
...
bash scripts/refresh-cask.sh "$dist" "$stage/orakul.rb" >/dev/null
...
ORAKUL_RELEASE_STAGE="$stage" \
ORAKUL_RELEASE_TAG="$tag" \
ORAKUL_RELEASE_VERSION="$version" \
ORAKUL_RELEASE_COMMIT="$commit" \
ORAKUL_RELEASE_SOURCE_HASH="$source_hash" \
ORAKUL_RELEASE_GENERATED_AT="$generated_at" \
python3 <<'PY'
import hashlib
import json
import os
...
stage = Path(os.environ["ORAKUL_RELEASE_STAGE"])
described = [
    ("orakul-AppleSilicon.dmg", "macos", "arm64"),
    ("orakul-Intel.dmg", "macos", "x86_64"),
    ("orakul.rb", "homebrew-cask", "multi"),
]
...
record = {
    "schemaVersion": 1,
    "project": "orakul",
    "version": os.environ["ORAKUL_RELEASE_VERSION"],
    "tag": os.environ["ORAKUL_RELEASE_TAG"],
    "commit": os.environ["ORAKUL_RELEASE_COMMIT"],
    "sourceSha256": os.environ["ORAKUL_RELEASE_SOURCE_HASH"],
    "generatedAt": os.environ["ORAKUL_RELEASE_GENERATED_AT"],
    "builder": {
        "workflow": workflow,
