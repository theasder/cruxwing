...
/// `CruxwingCore` — захват звука (WASAPI вместо ScreenCaptureKit) и оболочка.
...
struct PortabilityTests {
    ...
    /// цены будущего порта.
    ///
    /// `FoundationNetworking` появился здесь 2026-08-17 и не по недосмотру: на
    /// Linux и Windows swift-corelibs-foundation держит `URLRequest`,
    /// `URLSession` и `HTTPURLResponse` в отдельном модуле, и без него ядро там
    /// не собирается вовсе. Выяснилось сборкой в контейнере `swift:6.0`, а не
    /// чтением импортов: этот самый набор шесть коннекторов с `URLRequest`
    /// пропускал и отвечал «переносимо».
    static let allowed: Set<String> = ["Foundation", "FoundationNetworking"]
    ...
    private static var coreDirectory: URL {
        // От файла теста вверх до корня пакета: Tests/CruxwingCoreTests/<этот>.
        URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()   // CruxwingCoreTests
    ...
    func shellIsWhereThePlatformLives() throws {
        let shell = Self.coreDirectory
            ...
            .appendingPathComponent("CruxwingApp")
