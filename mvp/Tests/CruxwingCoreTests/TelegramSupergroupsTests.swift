    ...
    func validatesReadableBot() async throws {
        ...
        let client = TelegramSupergroups(token: "123:synthetic", http: http { request in
            ...
            if request.url!.path.hasSuffix("/getMe") {
                return #"{"ok":true,"result":{"id":44,"is_bot":true,"username":"cruxwing_test_bot","first_name":"Cruxwing","can_read_all_group_messages":true}}"#
            }
        ...
        #expect(bot.id == 44)
        #expect(bot.username == "cruxwing_test_bot")
