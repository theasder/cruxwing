...
///
///     CRUXWING_HOSTILE_PORT=4801 CRUXWING_COLLECTOR_PORT=4802 \
///     swift test --package-path mvp --filter HostileServiceTests
@Suite("Недружелюбный сервис по-настоящему", .serialized,
       .enabled(if: ProcessInfo.processInfo.environment["CRUXWING_HOSTILE_PORT"] != nil))
struct HostileServiceTests {

    static var vendorPort: String {
        ProcessInfo.processInfo.environment["CRUXWING_HOSTILE_PORT"] ?? "4801"
    }
    static var collectorPort: String {
        ProcessInfo.processInfo.environment["CRUXWING_COLLECTOR_PORT"] ?? "4802"
    }
