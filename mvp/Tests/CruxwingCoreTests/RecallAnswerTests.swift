...
struct RecallAnswerTests {
    ...
    func bothSurfacesPassTheFlag() throws {
        ...
        for (file, surface) in [("Sources/CruxwingCore/CommandLineApp.swift", "командная строка"),
                                ("Sources/CruxwingApp/CruxwingApp.swift", "приложение")] {
...
struct UnreadableArchiveTests {
    ...
    func missingDirectoryIsEmptyArchive() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("cruxwing-нет-\(UUID().uuidString)")
    ...
    func unreadableDirectoryIsNotEmptiness() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("cruxwing-закрыт-\(UUID().uuidString)")
    ...
    func wordingFollowsTheCause() {
        ...
        let broken = RecallAnswer.compose(query: "тарифы", hits: [],
                                          archiveIsEmpty: true, unreadable: [".cruxwing/"])
