...
struct ExternalTranscriberTests {
    ...
    func worksInsideThePipeline() async throws {
        let store = SessionStore(root: FileManager.default.temporaryDirectory
            .appendingPathComponent("cruxwing-ext-\(UUID().uuidString)", isDirectory: true))
...
struct TranscriberErrorTextTests {
    ...
    /// запись в архив, где вместо разговора байты. В CRUXWING_ENGINE легко
    ...
    @Test("настоящая расшифровка проходит",
          arguments: [
            ...
            "Борис: запусти `swift build -c release`, потом ./cruxwing найти тарифы",
