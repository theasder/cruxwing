...
/// Окно cruxwing: три вещи и ничего больше — записать, найти, прочитать ответ.
///
/// Интерфейс намеренно тонкий. Всё, что решает, ЧТО показать, живёт в
/// `CruxwingCore` и покрыто тестами; здесь только кнопки и текст, потому что
/// проверить SwiftUI-разметку тестом дорого, а сломать её незаметно — легко.
@main
struct CruxwingApp: App {
    var body: some Scene {
        WindowGroup("cruxwing") {
            ContentView()
    ...
    init() {
        let home = ProductEnvironment.value("HOME")
            ?? FileManager.default.homeDirectoryForCurrentUser
                .appendingPathComponent(".cruxwing").path
...
struct ContentView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            Text("Recording: cruxwing record 60 «Standup». System audio is not captured "
