...
// Тонкая оболочка: вся логика и все тексты живут в CruxwingCore, потому что здесь
...
let home = ProductEnvironment.value("HOME")
    ?? FileManager.default.homeDirectoryForCurrentUser.appendingPathComponent(".cruxwing").path
...
func recordFromMicrophone(_ rest: [String]) async -> CommandLineApp.Result {
    ...
    do {
        ...
        Next: cruxwing transcribe \(wav.path) "\(title.isEmpty ? "Call" : title)"
        """, exitCode: 0)
    } catch {
...
case "корпус", "corpus":
    ...
    if let directory = rest.first {
        ...
        result = CommandLineApp.Result(
            output: "Usage: cruxwing corpus <folder>. The description is corpus.json inside it.",
            exitCode: 2)
    }
case "спросить", "ask":
    ...
    if rest.count < 2 {
        result = CommandLineApp.Result(
            output: "Usage: cruxwing ask <service> <question>.\n"
                + "Services: " + ConnectorQuery.services.joined(separator: ", ") + ".",
            exitCode: 2)
    } else {
        // Поля, которые у сервиса свои: `CRUXWING_FIELD_workspace=…`. Имена не
        ...
        let fields = environment.reduce(into: [String: String]()) { result, pair in
            guard pair.key.hasPrefix("CRUXWING_FIELD_") else { return }
            result[String(pair.key.dropFirst("CRUXWING_FIELD_".count))] = pair.value
        }
        let settings = ConnectorQuery.Settings(
            service: rest[0],
            token: ProductEnvironment.value("TOKEN", environment: environment) ?? "",
            host: ProductEnvironment.value("HOST", environment: environment),
            scope: ProductEnvironment.value("SCOPE", environment: environment),
            values: fields)
        let answer = await ConnectorQuery.ask(settings,
                                              query: rest.dropFirst().joined(separator: " "))
        // Код возврата — не украшение: `cruxwing спросить … && развернуть`
...
// Ошибки — в stderr, чтобы `cruxwing найти ... > файл` не смешивал ответ с
