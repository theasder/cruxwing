...
/// Командная строка cruxwing: то, что можно запустить уже сегодня.
///
...
public struct CommandLineApp {
    ...
    public init(store: SessionStore,
                ...
                engineCommand: String? = ProductEnvironment.value("ENGINE"),
    ...
    cruxwing — search your own calls: Russian speech, no network

      cruxwing add <file> [title]           put an existing transcript in the archive
      cruxwing record [seconds] [title]     record from the microphone and transcribe
      cruxwing transcribe <wav> [title]     transcribe a recording into the archive
      cruxwing search <question>            what was decided about it
      cruxwing list                         what the archive holds
      cruxwing delete <id>                  remove one call
      cruxwing ask <service> <question>     ask a connected service
      cruxwing corpus <folder>              check a speech corpus before measuring

    The archive is plain JSON files: they can be read without us.
    Services: \(ConnectorQuery.services.joined(separator: ", ")).
    The token goes in CRUXWING_TOKEN. CRUXWING_HOST is the second thing a service asks
    for: the address of your own server, a Yandex Tracker organisation, a Kaiten
    team address. CRUXWING_SCOPE is where to look: a team in the messenger, the
    repositories for GitHub.
    Transcription runs on your engine: CRUXWING_ENGINE="whisper-cli -l ru -otxt -f {file}"
    """
    ...
    static func whyUnreadable(_ path: String) -> String {
        ...
        return "Could not read the file: \(path)\n"
            ...
            + "and if it is a call recording, transcribe it: cruxwing transcribe <wav>"
    }
    ...
    private func add(_ arguments: [String]) -> Result {
        guard let path = arguments.first else {
            return Result(output: "A transcript file is required: cruxwing add <file>", exitCode: 2)
        }
        ...
        if let existing = store.load().sessions.first(where: { $0.digest == text }) {
            ...
            Nothing was added. To see the archive: cruxwing list
    ...
    static func explain(_ error: Error) -> String {
        ...
        case 513, 257:
            return "No permission to write to the archive. Check the directory permissions "
                + "or name another one in CRUXWING_HOME."
        case 640:
            return "The disk is full."
        case 4, 260:
            return "Archive not found. Check CRUXWING_HOME."
        case 642:
    ...
    ///
    /// `cruxwing список` — построчный вывод: дата, идентификатор, название. Оно
    ...
    private func transcribe(_ arguments: [String]) -> Result {
        guard let path = arguments.first else {
            return Result(output: "A recording file is required: cruxwing transcribe <wav>", exitCode: 2)
        }
        ...
        guard let command = engineCommand, !command.isEmpty else {
            ...
            return Result(output: """
            No recognition engine is configured. cruxwing ships no model of its \
            own — it runs yours:

              export CRUXWING_ENGINE="whisper-cli -m model.bin -l ru -otxt -f {file}"
            """, exitCode: 2)
        }
    ...
    private func search(_ arguments: [String]) -> Result {
        ...
        guard !query.trimmingCharacters(in: .whitespaces).isEmpty else {
            return Result(output: "A question is required: cruxwing search what did we decide about pricing", exitCode: 2)
        }
    ...
    private func list() -> Result {
        ...
        guard !archive.sessions.isEmpty else {
            return Result(output: "The archive is empty. Add a transcript: cruxwing add <file>",
                          exitCode: 0)
        }
    ...
    private func delete(_ arguments: [String]) -> Result {
        guard let id = arguments.first else {
            return Result(output: "An id is required: cruxwing delete <id>",
                          exitCode: 2)
        }
        ...
        case .none:
            return Result(output: """
            No such call: \(id)
            To see what is here: cruxwing list
            """, exitCode: 1)
        }
