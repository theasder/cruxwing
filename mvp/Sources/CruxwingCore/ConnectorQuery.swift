...
public enum ConnectorQuery {
    ...
    /// Что можно спросить. Имена те же, что у `CRUXWING_PROBE_SERVICE`.
    ///
    ...
    /// всегда, и `cruxwing спросить … && развернуть` продолжал работу после
    ...
    public static func ask(_ settings: Settings,
        ...
        // токена стояла первой, и на `cruxwing спросить нетакого вопрос` продукт
        ...
        if service == "notes" {
            guard let folder = settings.host, !folder.isEmpty else {
                return .init(text: "Notes: a folder is required — put the path in CRUXWING_HOST.",
                             failed: true)
            }
            do {
        ...
        guard !settings.token.isEmpty else {
            return .init(text: "No token. Put it in CRUXWING_TOKEN — it is not written anywhere.",
                         failed: true)
        }
        ...
        do {
            if let service = RussianTrackers.Service(rawValue: settings.service) {
                ...
                if let prompt = service.secondaryPrompt, (settings.host ?? "").isEmpty {
                    return .init(text: "\(service.title): \(prompt) is required — put it in CRUXWING_HOST.",
                                 failed: true)
                }
            ...
            if settings.service == "github" {
                ...
                guard !repositories.isEmpty else {
                    return .init(text: "GitHub: \(GitHubConnector.repositoriesPrompt) are required — in CRUXWING_SCOPE.",
                                 failed: true)
                }
    ...
    private static func explain(_ error: Error) -> String {
        ...
        case .cannotFindHost, .cannotConnectToHost, .dnsLookupFailed:
            return "Could not reach the service\(host). Check the address in CRUXWING_HOST."
        ...
        case .secureConnectionFailed:
            ...
            return "The service\(host) refused the secure connection. Usually that means it "
                ...
                + "version. Check that the address in CRUXWING_HOST starts the same "
