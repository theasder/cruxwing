...
public struct RussianTrackers {
    ...
    static func splitBitrixWebhook(service: Service,
        ...
        // метода — его отбрасываем: адрес метода cruxwing подставляет сам.
    ...
    func body(for query: String, limit: Int) throws -> Data? {
        ...
        case .yandexTracker:
            ...
            // сводке и описанию. То есть вопрос человека попадает не в поле, а
            // в ВЫРАЖЕНИЕ, и это тот же случай, что у Jira, Slack и BookStack.
            //
            // Здесь стояла замена кавычки на апостроф. Экранированием она не
            // была ничем: апостроф в языке Трекера не служебный, а значит
            // строка просто МЕНЯЛАСЬ — человек, спросивший про «Избранное» в
            // кавычках, искал другое слово и не находил своё. И главное:
            // обратный слеш не трогался вовсе, а он экранирует следующий знак.
            // Вопрос, кончающийся слешем, съедал закрывающую кавычку, и остаток
            // выражения дописывал уже не человек.
            //
            // Вендор говорит прямо: `"` и `\` в точных запросах экранируются
            // обратным слешем. Порядок обязателен — сначала слеш, иначе
            // добавленный слеш сам будет экранирован следующим проходом.
            let escaped = query
