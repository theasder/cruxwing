...
///
/// Своей модели cruxwing не возит: веса весят гигабайты, а репозиторий, который
...
public struct ExternalTranscriber: Transcriber {
    ...
    public enum TranscriberError: Error, Equatable, CustomStringConvertible {
        ...
        public var description: String {
            switch self {
            case .commandIsEmpty:
                return "CRUXWING_ENGINE holds an empty command — there is nothing to run."
            case .commandHasNoFilePlaceholder:
                return "The command has no place for the file. Add {file} — "
                    + "cruxwing substitutes the recording's path there."
            ...
            case .engineSaidNothing:
                return "The engine ran but returned no text. Usually that is "
                    ...
                    + "the CRUXWING_ENGINE command on this same file by hand."
            case .engineSaidGibberish(let letters, let total):
                return "The engine returned something other than text: \(letters) letters out of \(total) characters. "
                    + "Usually CRUXWING_ENGINE holds the wrong program — a path to "
    ...
    public func transcribe(samples: [Float]) async throws -> String {
        ...
        let audio = FileManager.default.temporaryDirectory
            .appendingPathComponent("cruxwing-\(UUID().uuidString).wav")
    ...
    /// Случай не выдуманный: в CRUXWING_ENGINE легко указать не ту программу —
