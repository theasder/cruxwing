...
enum SharedDefaults {
    ...
    /// Занять задвижку на время тела теста и вернуть настройки как было.
    ///
    /// Существует, чтобы не переписывать каждый тест в две функции. Наборы,
    /// которые меняют общие настройки, обязаны ходить по очереди: `.serialized`
    /// у набора этого не даёт — он упорядочивает тесты ВНУТРИ набора, а наборы
    /// идут параллельно друг другу.
    ///
    /// Симптом, ради которого это написано: `BlindSpotSchedulerRaceTests`
    /// падал только на полном прогоне, поодиночке был зелёным, и падение
    /// указывало на планировщик — хотя причина в том, что соседний набор менял
    /// `brainstormEnabled` посреди ожидания.
    static func withConfigLock<T>(_ body: () async throws -> T) async rethrows -> T {
        try await withExclusiveAccess(body)
    }
