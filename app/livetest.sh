...
send ai.cruxwing.desktop.livetest.toggleRecording
sleep 5
dump
...
for BUTTON in summary tasks; do
    ...
    send ai.cruxwing.desktop.livetest.runPrompt id "$BUTTON"
    OK=1
...
send ai.cruxwing.desktop.livetest.toggleRecording; sleep 3
echo ">> analyzing persisted audio heartbeats since $TEST_START"
LOGS="$(log show --start "$TEST_START" --predicate 'subsystem == "ai.cruxwing.desktop"' 2>/dev/null)"
