...
sources) against Orakul's five tabs (General · Transcription · AI · Connected
...
Claude desktop is a general chat client; Orakul is a live-meeting co-pilot.
Several of Claude's settings solve problems Orakul does not have (multiple
...
two or three settings whose absence an Orakul user would actually feel.

## The table

| Claude desktop setting | Orakul today | Verdict | Reason |
|---|---|---|---|
| **Global hotkey** (Option+Space summons the app) | none — you alt-tab from Zoom to Orakul | **ADOPT** | The highest-value gap. A meeting co-pilot's whole premise is that you are looking at the *call*, not at it. A global hotkey to start/stop recording and to summon the assistant without leaving Zoom is directly on-mission. Put it in General → During calls. |
...
| **Keyboard shortcuts panel** (rebindable) | Fixed shortcuts (⌥⌘1-3 panes, item 3) | **ADAPT** | Orakul already ships pane/record shortcuts; surface them in a Settings → General list (discoverability), rebinding is optional and lower value. |
| **Extensions — browse & install** (.mcpb marketplace, Developer mode) | Curated `MCPCatalog` connectors via OAuth | **REJECT (install model), ADAPT (gallery)** | Orakul's connector strategy is depth-not-breadth (item 14); a `.mcpb` install marketplace is the opposite and a security surface. But a *browsable connector gallery* (what's available, one-click connect) is worth adapting from the "Browse extensions" idea. |
...
| **Appearance — theme** | Theme (Auto/Light/Dark) + reading text size | **ADOPTED** | Already shipped (items 2, and General → Appearance), and Orakul's reading-size control goes further. |
| **MCP data-access warnings / tool permissions** | Account & Privacy tab, outbound redaction (item 6), per-app mute | **ADOPTED / EXCEEDED** | Orakul's outbound redaction (cards, keys, JWTs, a user term list) and per-app mute are a *stronger* privacy posture than Claude's "be thoughtful about folders" note. Keep the copy in Account & Privacy that names it. |
| **Model selection** (in-chat picker) | Model selection among providers configured by the user | **ADOPTED** | Already present; availability follows the provider keys stored by the user in Keychain. |
...
meeting to reach the co-pilot). Everything else Orakul either already has
