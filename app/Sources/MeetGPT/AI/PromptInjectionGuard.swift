...
enum PromptInjectionGuard {
    ...
    /// Сторож смотрит СКВОЗЬ невидимое.
    ///
    /// Указание, записанное блоком Unicode Tags, не отображается ничем и не
    /// содержит ни одной буквы латиницы — сравнение строк не находило в нём
    /// ничего. Разворачиваем такие знаки перед сравнением: обнаружение обязано
    /// видеть сквозь запутывание, иначе достаточно записать то же самое
    /// невидимыми знаками. В запрос эта развёрнутая строка не уезжает.
    static func signal(in content: String) -> Signal? {
        scan(InvisibleText.unmaskTags(content))
    }
