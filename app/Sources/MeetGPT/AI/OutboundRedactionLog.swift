...
final class OutboundRedactionLog: ObservableObject {
    ...
    ///
    /// Без `unsafe`: компилятор сам видит, что тип Sendable, и вчерашняя
    /// пометка оказалась лишней — он об этом и сказал отдельным
    /// предупреждением.
    nonisolated static let shared = OutboundRedactionLog()
