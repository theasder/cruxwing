...
final class AudioChunkBuffer {
    ...
    /// Что показать человеку, если строк всё нет.
    ///
    /// «Слушаю. Строки появятся по ходу разговора» — фраза, верная ровно тогда,
    /// когда звук действительно идёт. Если устройство держит другое приложение
    /// (а во время выбора продукта рядом почти всегда работает чужой
    /// диктофон), или разрешение отозвали посреди звонка, буферы не приходят
    /// вовсе — и человек ждёт молча, уверенный, что всё в порядке.
    ///
    /// Различие здесь ровно то, ради которого счётчики и заведены: НОЛЬ
    /// буферов — это сломанный путь звука; буферы есть, а громкость под
    /// порогом — это тихая комната, и говорить о поломке нельзя. Про второе
    /// ничего не сообщаем: тишина в тихой комнате — не новость.
    static func trouble(buffersIn: Int, secondsListening: TimeInterval,
