...
enum CalendarError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .http(let code): return "Google Calendar answered \(code)."
        case .noEvent:        return "There are no calendar meetings near the current time."
        }
    }
}
...
enum CalendarService {
    ...
    static func currentAgenda(accessToken: String,
                              ...
                              session: URLSession = CruxwingNetworkIdentity.shared) async throws -> CalendarAgenda {
        let now = Date()
    ...
    static func upcomingEvents(accessToken: String,
                               ...
                               maxResults: Int = 25,
                               session: URLSession = CruxwingNetworkIdentity.shared) async throws -> [UpcomingMeeting] {
        let now = Date()
        let timeMin = ISO8601DateFormatter().string(from: now)
