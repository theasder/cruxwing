...
enum PromptWorkflows {
    ...
    /// Похож ли поисковый запрос на ЦИТАТУ из встречи.
    ///
    /// Запрос, собранный моделью из расшифровки, уезжает в каждое подключённое
    /// приложение — включая сервис конкурента, который и продаёт расшифровки.
    /// Отправить «тарифы декабрь Германия» — это поиск. Отправить «мы решили
    /// поднять тарифы с декабря на пятнадцать процентов для клиентов из
    /// Германии» — это отдать содержание встречи тому, у кого его быть не
    /// должно. Продукт обещает, что запись остаётся на этом компьютере; длинная
    /// дословная строка это обещание нарушает молча.
    ///
    /// Проверяется ПОДРЯД идущий общий кусок, а не совпадение вообще: отдельные
    /// слова встречи в запросе не просто допустимы — ради них он и существует.
    ///
    /// Порог — шесть слов подряд, и только он.
    ///
    /// В первой редакции рядом стоял порог по знакам: сорок. Он мерил длину, а
    /// не содержание, и пропускал настоящую фразу — «тогда пересчитаю лимиты и
    /// вернусь в» это тридцать пять знаков, потому что служебные слова в
    /// русском короткие. Замена на счёт содержательных слов оказалась почти
    /// бездействующей: у любых шести слов живой речи их набирается три, и
    /// мутация, снявшая этот счёт, набор проходила. Условие, которое почти
    /// никогда не срабатывает, — это не строгость, а вид строгости.
    ///
    /// Осталось одно правило, которое можно объяснить вслух: шесть слов подряд,
    /// совпавших с расшифровкой, — это цитата. Ошибиться в эту сторону дёшево:
    /// запрос откатывается к цели звонка, поиск становится шире. Ошибиться в
    /// другую — значит отдать сказанное на встрече чужому сервису.
    static func looksLikeAQuote(_ query: String, of transcript: String,
                                minimumWords: Int = 6) -> Bool {
        let words = { (text: String) in
            text.lowercased()
                .split(whereSeparator: { !$0.isLetter && !$0.isNumber })
                .map(String.init)
        }
        let needle = words(query)
        guard needle.count >= minimumWords else { return false }
        let haystack = words(transcript)
        guard haystack.count >= minimumWords else { return false }

        for start in 0...(needle.count - minimumWords) {
            let run = Array(needle[start..<(start + minimumWords)])
            for begin in 0...(haystack.count - minimumWords)
            where Array(haystack[begin..<(begin + minimumWords)]) == run {
                return true
            }
        }
        return false
